...
def trim_silence(audio, frame_size=1024, hop_size=512, threshold=0.01):
    ...
    if not kept_frames:
        return audio  # failsafe, avoid empty
...
def analyze_track(file_path, output_dir, track_type):
    ...
    stereo_audio = mono_to_stereo(trimmed_audio)

    # Check for Noise Issues
    noise_issues = detect_noise_floor(trimmed_audio)

    # Extract features
    lufs_result = es.LoudnessEBUR128()(stereo_audio)[0]
    integrated_lufs = lufs_result[0].item()

    rms = es.RMS()(trimmed_audio)
    ...
    problem_freqs = detect_problem_frequencies(trimmed_audio)
    ...
    window_algo = es.Windowing(type="hann")
    for start in range(0, len(trimmed_audio) - frame_size, 512):
        frame = audio[start: start + frame_size]
    ...
    trimmed_duration = len(trimmed_audio) / 44100
    original_duration = len(audio) / 44100

    percent_active = (trimmed_duration / original_duration) * 100

    console.print(f"🎤 Active Signal: {percent_active:.1f}% of the track")
    # Generate report
    report = {
        "file": file_path,
